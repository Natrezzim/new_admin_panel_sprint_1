...
class SQLiteLoader(str):
    ...
    def load_movies(self, sqlite_conn: str):
        ...
        for row in load_from_table('film_work', fields_film_work, sqlite_conn, 100):
            film_work.append(FilmWork(row['id'],row['title'],row['description'],row['creation_date'],row['rating'], row['type'], row['created_at'], row['updated_at']))
        ...
        for row in load_from_table('person_film_work', fields_person_film_work, sqlite_conn, 100):
            person_film_work.append(PersonFilmWork(row['id'], row['person_id'], row['film_work_id'], row['role'], row['created_at']))
